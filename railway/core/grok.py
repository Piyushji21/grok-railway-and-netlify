...
class Grok:
    ...
    def _load(self, extra_data: dict = None) -> None:
        ...
        if not extra_data:
            ...
            self.actions, self.xsid_script = Parser.parse_grok(scripts, self.session.proxies.get("all") if self.session.proxies else None)
        ...
        if self.c_run == 0:
            ...
            match self.c_run:
                ...
                case 2:
                    ...
                    self.svg_data, self.numbers = Parser.parse_values(c_request.text, self.anim, self.xsid_script, self.session.proxies.get("all") if self.session.proxies else None)
